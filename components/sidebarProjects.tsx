    ...
    return (
        <div className="sm:border-r-2 sm:max-w-xs w-full border-color-2 pl-5 pt-5 sm:block absolute sm:static z-20 backdrop-blur-md sm:backdrop-blur-0 pb-4">
            ...
            <motion.div className="space-y-4 overflow-hidden h-0"
                ...
                {categories.map(category => <FilterInput key={category.id} name={category.name} icon={category.icon} id={category.id} setSelected={setSelected} />)}
            </motion.div>
        </div>
    )
}

function FilterInput({ label, name, value, icon, id, setSelected }: { label?: string, name: string, value?: string, icon: string, id: number, setSelected: any }) {
    return (
